...
<style>
    ...
    .data-summary-box h2, .data-summary-box h3 {
        color: white !important;
    }
    .explanation-box {
        background: #e8f4f8;
    ...
    /* Markdown in colored boxes should be white */
    .description-box *, .metric-box *, .data-summary-box * {
        color: white !important;
    }
    .description-box h3, .metric-box h2, .metric-box h3, .data-summary-box h2, .data-summary-box h3 {
        color: white !important;
    }
    .description-box p, .description-box strong, .description-box b {
        color: white !important;
    }
    .metric-box p, .metric-box strong, .metric-box b {
        color: white !important;
    }
    .data-summary-box p, .data-summary-box strong, .data-summary-box b {
        color: white !important;
    }
    /* But keep info-box and explanation-box dark */
    .info-box *, .explanation-box * {
        color: #1a1a1a !important;
    }
    /* Welcome screen colored divs */
    ...
    /* Streamlit metrics in colored boxes */
                ...
                with st.spinner("🔄 Training model..."):
                    try:
                        ...
                        if target_is_categorical:
                            ...
                            # Store class names
                            st.session_state.class_names = le_target.classes_
                        else:
                            y_encoded = y
...
if st.session_state.model is not None:
    ...
    is_classification = len(np.unique(st.session_state.y_test)) < 20
    
    if is_classification:
        # Use different averaging strategies to get varied metrics
        ...
        with col5:
            ...
            <div class="metric-box">
                <h3>📊 Features</h3>
                <h2>{len(st.session_state.feature_names)}</h2>
            </div>
            """, unsafe_allow_html=True)
    ...
    with col_diag2:
        ...
        sns.heatmap(corr_matrix, 
                    ...
                    annot_kws={'size': 8, 'weight': 'bold'})  # Larger, bold text
    ...
    with tab2:
        ...
        with st.spinner("🔄 Computing SHAP values..."):
            try:
                ...
                sample_encoded = st.session_state.X_test.iloc[sample_idx]
    ...
    with tab3:
        ...
        with col_lime2:
            # Safely get actual class - handle both Series and array-like objects
            if hasattr(st.session_state.y_test, 'iloc'):
        ...
        with st.spinner("🔄 Generating LIME explanation..."):
            try:
                ...
                num_classes = len(pred_proba_lime)
                
                if num_classes > 2:
                    st.info(f"📊 Showing LIME explanations for all {num_classes} classes")
                    ...
                    for class_idx, lime_tab in enumerate(lime_tabs):
                        with lime_tab:
                            ...
                            exp_class = explainer_lime.explain_instance(
                                ...
                                st.session_state.model.predict_proba,
                                num_features=len(st.session_state.feature_names),
                                labels=[class_idx]  # Specify which class to explain
                            )
                    ...
                    # Binary classification - show for predicted class
    ...
    with tab4:
        ...
        with col1:
            ...
            try:
                explainer = shap.TreeExplainer(st.session_state.model)
                shap_values_comp = explainer.shap_values(st.session_state.X_test[:100])
                
                # Determine number of classes
                num_classes = len(pred_proba_comp)
                
                if num_classes > 2 and isinstance(shap_values_comp, list):
                    st.info(f"📊 Showing SHAP waterfall for all {num_classes} classes")
